...
**Cumulus.nvim** is a customized, thematic, and highly stable JVM IDE built purely on the Neovim ecosystem with cloud superpowers. Its primary goal is to fully replace IntelliJ IDEA for daily, mission-critical enterprise backend development in Java and Kotlin.
...
## 5. Out of Scope
- Building or maintaining custom backend language engines (the legacy Scala `cumulus-engine` is deprecated).
