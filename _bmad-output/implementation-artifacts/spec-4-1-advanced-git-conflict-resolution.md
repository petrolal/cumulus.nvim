...
**Problem:** tetravim.nvim has no visual 3-way merge-conflict resolver and no way to explore history for a specific code block or line range. A mid-merge developer must hand-edit `<<<<<<<`/`=======`/`>>>>>>>` markers, and `gitsigns` only offers whole-file blame/diff. This is the first gap Epic 4 ("workflow parity") targets.
...
- New plugin lives in its own `lua/tetravim/plugins/tools-diffview.lua`, `cmd`/`keys`-lazy-loaded, following `tools-http.lua`'s isolation shape.
...
- Guard every entry point: not inside a git work tree, or `git` not executable → clear message via `tetravim.util.ui.notify_err` and stop. Never a raw stacktrace.
- Reuse `gitsigns`' `<leader>gb` / `<leader>gB` for line blame; add no second blame path.
- Register a `:checkhealth tetravim` section for git presence/version and `diffview.nvim` loadability, following the existing `health.lua` section shape.
- Add a `lua/tetravim/tests/*_spec.lua` (static shape only — must NOT `require` the lazy plugin) plus `scripts/validate-4-1.sh` runtime smoke test, per `AGENTS.md`.
...
**Never:**
- Do not touch the legacy `tetravim.util.engine` `parse_git_conflicts` / `resolve_git_conflicts` stub (out of scope).
...
| `git` binary missing | keymap / `:checkhealth tetravim` | keymap: `notify_err` + hint; health: `vim.health.error` | guarded |

</frozen-after-approval>

## Code Map

- `lua/tetravim/plugins/tools-http.lua` -- isolation reference: single-feature plugin file, `cmd`/`keys` lazy-load, `opts` + `config`, keymap cluster. Copy this shape for `tools-diffview.lua`.
- `lua/tetravim/plugins/tools-gitsigns.lua:16-55` -- existing `<leader>g` keymaps: `gb`/`gB` blame (reuse), `gd` diffthis, `gh*` hunks, `]c`/`[c` nav. Keep clear of these sub-keys.
- `lua/tetravim/plugins/editor-snacks.lua:311-338` -- other `<leader>g` bindings (`gg gl gL gs gS`); `gc` and `gx` sub-keys are free.
- `lua/tetravim/plugins/ui-whichkey.lua:21-35` -- `vim.list_extend(opts.spec, { … })` block where `<leader>` groups register (`g` = "git control" ~:28). Add the `<leader>gc` group entry here only.
- `lua/tetravim/core/keymaps.lua:148-241` -- precedent for a feature keymap cluster (`<leader>H*`). Prefer declaring new keys in the diffview spec's `keys`.
- `lua/tetravim/util/ui.lua:13-42` -- `notify_info/notify_warn/notify_err` (title "TetraVim"). Use these, never raw `vim.notify`.
- `lua/tetravim/util/http.lua:44` -- guard + `vim.system` style to model `util/git.lua` on.
- `lua/tetravim/health.lua:39` (`required_binaries = { rg, git }`), section list ends ~:245 -- add the new `vim.health.start(...)` after the HTTP client section; `scripts/validate.sh:24-30` loads this module.
- `lua/tetravim/util/engine.lua:664,1208-1232` -- legacy conflict stub. Read-only: leave untouched, do not call.
- `lua/tetravim/tests/profiling_spec.lua` -- busted static-spec shape for `git_conflict_spec.lua`. `scripts/validate-http.sh` -- smoke-test model (`cquit 1` on failure). `stylua.toml` -- 2-space, 120 col.

## Tasks & Acceptance

**Execution:**
- [x] `lua/tetravim/util/git.lua` (NEW) -- `M.in_worktree()` (via `git rev-parse --is-inside-work-tree`) and `M.guard()` returning `false` + `notify_err` when `git` is not executable or CWD is not a work tree, else `true`. Model on `util/http.lua`'s guard. Reused by Story 4.2.
- [x] `lua/tetravim/plugins/tools-diffview.lua` (NEW) -- `sindrets/diffview.nvim` spec: `dependencies = { "nvim-lua/plenary.nvim" }`, lazy on `cmd = { "DiffviewOpen", "DiffviewClose", "DiffviewFileHistory", "DiffviewToggleFiles", "DiffviewFocusFiles" }` + a `keys` cluster (Design Notes); `opts` sets a 3-way merge layout (`view.merge_tool.layout`, e.g. `"diff3_mixed"`) and rebinds conflict-resolution actions via `opts.keymaps` under `<leader>g`; each keymap callback runs `require("tetravim.util.git").guard()` first.
- [x] `lua/tetravim/plugins/ui-whichkey.lua` -- add `{ "<leader>gc", group = "conflict/compare", icon = "" }` to the `vim.list_extend(opts.spec, { … })` block (:21-35); leave every other group unchanged.
- [x] `lua/tetravim/health.lua` -- add a `vim.health.start("TetraVim Advanced Git Conflict Resolution")` section (after the HTTP client section): `vim.fn.executable("git")`, `git --version` >= 2.30, `pcall(require, "diffview")`; existing `ok/warn/error` shape.
- [x] `lua/tetravim/tests/git_conflict_spec.lua` (NEW) -- busted static spec: `tools-diffview.lua` first spec is `"sindrets/diffview.nvim"` and declares `cmd` + `keys`; `tetravim.util.git` exposes `in_worktree` and `guard`; which-key spec contains a `<leader>gc` group; `health.lua` source registers the new section. Must NOT `require("diffview")`.
- [x] `scripts/validate-4-1.sh` (NEW) -- fresh `nvim --headless -u init.lua`: `Lazy! load diffview.nvim`; assert `pcall(require, "diffview")`, `:DiffviewOpen` exists, the `<leader>gco`/`<leader>gch` mappings resolve, `:checkhealth tetravim` output contains the new section; in a `mktemp` repo with a real merge conflict, `:DiffviewOpen` → assert a diffview tabpage opened → `:DiffviewClose`; `cquit 1` on any failure.
...
- Given `:checkhealth tetravim` is run, then it reports an "Advanced Git Conflict Resolution" section covering git and `diffview.nvim`.
...
- `nvim --headless -u init.lua -c "PlenaryBustedDirectory lua/tetravim/tests/" -c "qa"` -- expected: `git_conflict_spec.lua` passes with the suite.

**Manual checks:**
- `:checkhealth tetravim` shows the "Advanced Git Conflict Resolution" section: OK when `git` and `diffview.nvim` are present, error/warn otherwise.
...
- Entry point: diffview.nvim is the sole engine, lazy-loaded on its `:Diffview*` commands and the `<leader>gc*` cluster.
  [`tools-diffview.lua:46`](../../lua/tetravim/plugins/tools-diffview.lua#L46)

- The golden keymap set — every lhs under `<leader>g`; `gco` opens the merge tool / working-tree diff.
  [`tools-diffview.lua:56`](../../lua/tetravim/plugins/tools-diffview.lua#L56)

- 3-way layout: `diff4_mixed` gives a distinct always-visible BASE pane (OURS / BASE / THEIRS).
  [`tools-diffview.lua:123`](../../lua/tetravim/plugins/tools-diffview.lua#L123)
...
- diffview's default `<leader>c*` and `dx`/`dX` picks disabled, then rebound under `<leader>gx*` / `<leader>gX*`.
  [`tools-diffview.lua:127`](../../lua/tetravim/plugins/tools-diffview.lua#L127)
...
- Shared git work-tree guard — `git` missing or non-repo CWD → one clear `notify_err`, timeout-bounded probe.
  [`git.lua:45`](../../lua/tetravim/util/git.lua#L45)

- History keymaps additionally bail (warn, not error) on unnamed / non-file buffers.
  [`tools-diffview.lua:23`](../../lua/tetravim/plugins/tools-diffview.lua#L23)

- Close / toggle-files act only on an already-open view — silent no-op otherwise, not an entry point.
  [`tools-diffview.lua:37`](../../lua/tetravim/plugins/tools-diffview.lua#L37)

**Discoverability & health**

- which-key groups for `<leader>gc` (conflict/compare) and `<leader>gx` (conflict: pick side).
  [`ui-whichkey.lua:29`](../../lua/tetravim/plugins/ui-whichkey.lua#L29)

- `:checkhealth tetravim` section: git presence + advisory `>= 2.30` version, diffview.nvim resolvability; honest warn on a failed probe.
  [`health.lua:245`](../../lua/tetravim/health.lua#L245)

**Tests**

- Static shape spec — declares diffview lazily, must never eagerly `require("diffview")`; repo-root-relative file reads.
  [`git_conflict_spec.lua:23`](../../lua/tetravim/tests/git_conflict_spec.lua#L23)
