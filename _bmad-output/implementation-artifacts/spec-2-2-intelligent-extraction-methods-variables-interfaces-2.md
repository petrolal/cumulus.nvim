...
- `lua/tetravim/core/devops.lua` -- ensure test panics are resolved without polluting the public `M` table with `resolve_search_dir` unless strictly necessary.
- `lua/tetravim/util/jvm.lua:436-486` -- remove legacy `<leader>jxm`, `<leader>jxv`, `<leader>jxc` immediate-apply commands.
- `lua/tetravim/util/extract.lua` -- add `M.extract_method()`, `M.extract_variable()`, and `M.extract_constant()`. Ensure `do_action` filters properly by specific kind or title.
- `ftplugin/java.lua:68` -- map the new extractions to `<leader>cm`, `<leader>cv`, and `<leader>cc` in `n` and `v` modes.
- `lua/tetravim/plugins/lsp-kotlin.lua:32` -- mirror keymaps for Kotlin LS (`n` and `v`).
- `lua/tetravim/tests/extract_spec.lua` -- verify both `n` and `v` mode keymaps exist via `maparg`.
...
- [x] `lua/tetravim/util/jvm.lua` -- delete old legacy extraction logic.
- [x] `lua/tetravim/util/extract.lua` -- implement Method, Variable, and Constant extractions.
- [x] `ftplugin/java.lua` & `lua/tetravim/plugins/lsp-kotlin.lua` -- wire `n` and `v` mode keymaps.
- [x] `lua/tetravim/tests/extract_spec.lua` -- assert new module shape and BOTH `n`/`v` keymaps.
...
- Replicated symmetric Kotlin LSP Normal (`n`) and Visual (`v`) code-action bindings natively.
  [`lsp-kotlin.lua:90`](../../lua/tetravim/plugins/lsp-kotlin.lua#L90)

- Stripped unmaintained, legacy Java extraction stubs relying on raw client implementations.
  [`jvm.lua:262`](../../lua/tetravim/util/jvm.lua#L262)
...
- Engine DevOps isolation hoisted to prevent downstream validation faults on root scans.
  [`devops.lua:56`](../../lua/tetravim/core/devops.lua#L56)
...
- [x] [Review][Patch] Resolve-failure path read `res.error` instead of `res.err` [lua/tetravim/util/extract.lua] — **Fixed:** `proceed_with_action` now reads `(res.err or res.error).message`, and the command-only branch guards `local caps = jvm_client.server_capabilities or {}`.
- [x] [Review][Patch] Extract Interface preview omitted the created interface file [lua/tetravim/util/extract.lua] — **Fixed:** new `resource_op_rows()` enumerates `documentChanges[]` `create`/`rename`/`delete` as `[new file] …` / `[rename] … -> …` / `[delete] …` quickfix rows; `_show_preview` merges them with the text locations and only aborts ("no changes returned") when the combined list is empty.
- [x] [Review][Patch] `action_lock` leaked on a synchronous throw between `acquire()` and the async request [lua/tetravim/util/extract.lua] — **Fixed:** the whole setup body (`make_range_params` → visual-mark block → `params.context` → `defer_fn` → `buf_request_all`) is wrapped in `pcall`, releasing the lock + `notify_err` on failure; the scheduled `handle_action_response` is `pcall`-wrapped with the same release-on-throw.
- [x] [Review][Patch] Linewise-visual `'>` MAXCOL not clamped [lua/tetravim/util/extract.lua] — **Fixed:** a local `clamp(row, col)` bounds both `'<` and `'>` byte columns to `[0, #line]` before `character_offset`, so a `V` selection's `2147483647` column can't produce an out-of-range call.
...
- [x] [Review][Patch] Kotlin keymap test never invoked the callbacks [lua/tetravim/tests/extract_spec.lua] — **Fixed:** the Kotlin `on_attach` test now stubs the five `extract.*` functions, invokes every `mapping.callback()`, and asserts the correct function ran + `is_visual=true` for visual variants (mirroring the Java `assert_mapping_calls`); added the missing visual-mode `<leader>ce` assertion on both sides; corrected the Java helper's `assert.are.equal(bufnr, mapping.buffer)` to `assert.are.equal(1, mapping.buffer)` (it's a 0/1 locality flag).
- [x] [Review][Patch] `extract.lua` stylua diff [lua/tetravim/util/extract.lua] — **Fixed:** `stylua` run; the `nvim_feedkeys` line now uses double quotes; `stylua --check` clean.

- [x] [Review][Defer] `codeAction/resolve` command-without-edit path has zero test coverage [lua/tetravim/util/extract.lua:28] — deferred; add a deterministic mock-seam step to `validate-extract.sh` (mock `buf_request_all` to return a command-only action for `textDocument/codeAction` and `{result={edit=…}}` for `codeAction/resolve`; assert apply + lock release), plus a no-`resolveProvider` case.
- [x] [Review][Defer] JDTLS-English-title matching is locale/server-specific; Kotlin LS emits different titles [lua/tetravim/util/extract.lua:279] — deferred, already recorded twice in `deferred-work.md`. `<leader>cm/cv/cc` will likely report "No applicable code action" on every Kotlin buffer, so the finalization AC ("Given a Java/**Kotlin** buffer, invoking `<leader>cm/cv/cc` … a preview is correctly presented") is not met for Kotlin.
- [x] [Review][Defer] `params.context.only` may be too specific [lua/tetravim/util/extract.lua:239] — deferred; `{ "refactor.extract.interface" }` / `{ "refactor.inline" }` won't match a server that tags the action with the shorter parent kind.
- [x] [Review][Defer] Actions with `kind == nil` are silently dropped [lua/tetravim/util/extract.lua:97] — deferred; `action.kind and vim.startswith(...)` skips a correctly-titled action that omits `kind` (some servers do).
- [x] [Review][Defer] Normal-mode `<leader>cm/cv/cc` operate on a zero-width range [lua/tetravim/util/extract.lua:199] — deferred; `make_range_params` in normal mode is the cursor position, so JDTLS extract-method/variable is mostly a no-op there; visual `<leader>ce` similarly passes a meaningless range for a whole-type refactor.
- [x] [Review][Defer] `nvim_feedkeys('<Esc>','x')` to refresh `'<`/`'>` is version-sensitive [lua/tetravim/util/extract.lua:209] — deferred; in a `<Cmd>` visual mapping the marks reflect the previous selection; the forced `<Esc>` flush is fragile — verify against the minimum supported Neovim or read the range before leaving visual.
- [x] [Review][Defer] New Neovim-version assumptions undocumented [lua/tetravim/util/extract.lua:199] — deferred; 2-arg `make_range_params` (0.10+, deprecated 0.11) and `character_offset` are used with no min-version note or `pcall` guard.
- [x] [Review][Defer] Dismissed `vim.ui.select` strands the shared lock with no recovery [lua/tetravim/util/extract.lua:122] — deferred; after the outer request timeout clears (`responded = true`) there is no stage-timeout while the confirm/disambiguation prompt is open, and `action-lock.lua` has no `force_release` / user command. Same cross-cutting item as the SPEC-2.1 `action_lock` recovery defer.
- [x] [Review][Defer] `_show_preview` calls `copen` unconditionally and never `cclose`s [lua/tetravim/util/extract.lua:152] — deferred; steals a window before confirm and leaves stale preview lists on Apply/Cancel/error.
- [x] [Review][Defer] Disambiguation menu shows identical titles [lua/tetravim/util/extract.lua:115] — deferred; `vim.ui.select` is fed only `action.title`, so two "Extract to method" entries are indistinguishable; append the target range/scope.
- [x] [Review][Defer] `diagnostics` context samples only the start line [lua/tetravim/util/extract.lua:238] — deferred; `vim.diagnostic.get(bufnr, { lnum = params.range.start.line })` ignores diagnostics elsewhere in a multi-line selection.
...
- [x] [Review][Defer] `devops.lua` `resolve_search_dir` hoist bundled into an extraction feature [lua/tetravim/core/devops.lua] — deferred; a legitimate forward-reference fix but off-topic for SPEC-2.2; `validate-devops.sh` now covers declaration order + finder callability.
- [x] [Review][Defer] `<leader>jx` which-key group now labels only `jxo`/`jxH` [lua/tetravim/util/jvm.lua:30] — deferred; relabel "refactor & jdtls" or leave — the group still has members.
...
- [x] [Review][Defer] Cosmetic split identity [lua/tetravim/util/extract.lua:9] — deferred; `extract.lua` notify title vs `refactor.lua`'s; `ACTION_TIMEOUT_MS/1000` renders "10.0s"; `buf_request_all` fans to all clients but only `jvm_client.id` is read.
