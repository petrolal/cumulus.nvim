...
- `lua/tetravim/util/profiling.lua` -- New module to encapsulate the profiling logic (commands to run `async-profiler`, track state, generate output).
- `lua/tetravim/util/jvm.lua` -- Register keybindings to trigger the profiling commands.
- `lua/tetravim/tests/profiling_spec.lua` -- Add a busted test to verify the public API shape.
...
**Execution:**
- [x] `lua/tetravim/util/profiling.lua` -- Create module providing start/stop/view APIs. Ensure robustness by checking if active PID already exists, parsing exit codes when using system fallback, verifying directory writability, and preserving previous flamegraph states on error.
- [x] `lua/tetravim/util/jvm.lua` -- Register keybindings to trigger the profiling commands.
- [x] `lua/tetravim/tests/profiling_spec.lua` -- Create unit test to verify module API shape.
...
- **2026-08-25**: (Loop 1) Review found that `async-profiler` is not a Neovim plugin, so placing it in `lua/tetravim/plugins/tools-profiling.lua` caused an invalid lazy.nvim spec. Also, test coverage for the keymaps and module shape was missing.
  - **Amended**: Removed `tools-profiling.lua` from Code Map/Tasks. Pointed to `lua/tetravim/util/profiling.lua` instead. Added tasks for `profiling_spec.lua` and `scripts/validate.sh` checks. Added edge-case handling instructions to the execution task for `profiling.lua`.
...
- Basic Busted suite enforcing module interface stability
  [`profiling_spec.lua:3`](../../lua/tetravim/tests/profiling_spec.lua#L3)
