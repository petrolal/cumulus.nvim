...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-3-native-spring-boot-discovery-legacy-engine-deprecation.md`
  ...
  summary: Full decommission of the remaining legacy engine surface — COMPLETE. `lua/tetravim/util/engine.lua` and every reference to the `tetravim-engine` binary have been deleted across source, tests, scripts, AI context, docs, and BMAD planning artifacts. Genuinely-native helpers were relocated (`util/notify.lua`, `util/term.lua`; native `vim.fs` marker-based root discovery in `core/devops.lua`; build-tool detection via `vim.fs.find`); binary-only features with no native path (unified IaC validators, workspace classification, engine metrics/refresh commands, `:TetraVimInstallEngine`) were removed. Coverage remains a self-contained native module (`util/coverage.lua`).
  evidence: User directive 2026-09-03 ("we need to decommission all the scala legacy engine"). Scala source was removed in `76bae38`; the Lua shim + prebuilt-binary dependency are now gone as well.
