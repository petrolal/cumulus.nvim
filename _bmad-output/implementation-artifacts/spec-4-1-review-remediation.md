...
- `<leader>gx` / `<leader>gX` which-key groups register buffer-locally for diffview buffers only, following `lua/cumulus/core/lang-keymaps.lua`'s `buffer = true` pattern; `<leader>gc` stays a global group.
...
- Do not hand-edit `lazy-lock.json`; do not touch `cumulus.util.engine`'s legacy conflict stub; no keymap outside `<leader>g`.
...
- Given diffview is not installed at all, when `:checkhealth cumulus` runs, then it reports an error telling the user to run `:Lazy install`, distinct from the not-yet-loaded warning; plenary absence is also reported.
- Given `stylua --check lua/ ftplugin/ init.lua`, then the changed files pass unchanged.
- Given `nvim --headless -u init.lua -c "PlenaryBustedDirectory lua/cumulus/tests/" -c "qa"`, then `git_conflict_spec.lua` passes and still never loads `diffview`.
...
- `nvim --headless -u init.lua -c "PlenaryBustedDirectory lua/cumulus/tests/" -c "qa"` -- expected: `git_conflict_spec.lua` green.
...
- `nvim --headless -u init.lua -c "checkhealth cumulus" -c "qa"` -- expected: no Lua errors; the Story 4.1 section renders.
