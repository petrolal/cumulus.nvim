...
/**
 * OCI Red theme palette and highlight generation.
 * Base color: #FF2B2B (OCI Red)
 */
object OciTheme:

  val BaseColor = "#FF2B2B"  // OCI Red
  private val ErrorColor = "#FF3333"
  private val WarningColor = "#FFB800"
  private val SuccessColor = "#27AE60"
  private val InfoColor = "#3498DB"
  ...
  def generateHighlights(): Map[String, HighlightGroup] =
    ...
    // Base primary colors with shades
    ...
    // Secondary shades (adjusted red)
    val secondaryBase = ColorPalette.generateShadeWithLightness(BaseColor, -0.15)
    val secondaryShades = ColorPalette.generateShades(secondaryBase)
    ...
    highlights("Function") = HighlightGroup(fg = Some("#FFA07A"))
