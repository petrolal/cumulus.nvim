-- TetraVim Scala Engine Integration (SPEC-031)
--
-- Discovers and executes the compiled `tetravim-engine` Scala binary to offload
...
--- Locates the `tetravim-engine` binary on $PATH, within engine/ target directory, or in user data dir.
---@return string|nil
function M.get_bin()
  ...
  -- 1. Check $PATH
  if vim.fn.executable("tetravim-engine") == 1 then
    cached_bin = "tetravim-engine"
  ...
  local local_native_bin = script_dir .. "/engine/target/native-image/tetravim-engine"
  local local_graalvm_bin = script_dir .. "/engine/target/graalvm-native-image/tetravim-engine"
  ...
  -- 3. Check installed binary location (~/.local/share/nvim/tetravim/bin/tetravim-engine)
  local data_bin = vim.fn.stdpath("data") .. "/tetravim/bin/tetravim-engine"
...
function M.assert_available(category)
  ...
  local error_msg = string.format(
    "tetravim-engine binary not found for [%s] operations\n"
      .. "\n"
      .. "Build with:\n"
      .. "  cd /path/to/tetravim.nvim/engine && sbt nativeImage\n"
      .. "\n"
      .. "Or install via:\n"
      .. "  :TetraVimInstallEngine\n"
      .. "\n"
      .. "Docs: https://github.com/petrolal/tetravim.nvim/docs/engine-setup",
    category_label
  )
...
--- Detects the host platform OS and architecture for tetravim-engine binary.
---@return string|nil target_name Target binary name (e.g., 'tetravim-engine-linux-x86_64') or nil
...
function M.detect_platform()
  ...
  if sysname == "Linux" then
    if machine == "x86_64" then
      return "tetravim-engine-linux-x86_64", nil
    elseif machine == "aarch64" or machine == "arm64" then
      return "tetravim-engine-linux-aarch64", nil
    end
  elseif sysname == "Darwin" then
    if machine == "arm64" or machine == "aarch64" then
      return "tetravim-engine-darwin-arm64", nil
    end
  end
...
--- Downloads and installs the pre-built tetravim-engine binary from GitHub Releases.
...
function M.install(opts, callback)
  ...
  if not target_name then
    local msg = string.format("[tetravim] %s", err or "Unsupported platform")
  ...
  if vim.fn.executable("curl") ~= 1 then
    local msg = "[tetravim] 'curl' is required to download tetravim-engine but was not found in PATH"
    vim.notify(msg, vim.log.levels.ERROR)
    cb(false, msg)
    return
  end

  local base_url = opts.url or "https://github.com/petrolal/tetravim.nvim/releases/latest/download/"
  ...
  vim.notify(string.format("[tetravim] Downloading %s...", target_name), vim.log.levels.INFO)
  ...
  vim.system({ "curl", "-fsSL", checksums_url, "-o", temp_checksums }, {}, function(res_sum)
    if res_sum.code ~= 0 then
      ...
      vim.schedule(function()
        vim.notify("[tetravim] " .. err_msg, vim.log.levels.ERROR)
        cb(false, err_msg)
      end)
      return
    end
    ...
    vim.system({ "curl", "-fsSL", bin_url, "-o", temp_bin }, {}, function(res_bin)
      ...
      vim.system(hash_cmd, { text = true }, function(res_hash)
        ...
        if actual_hash ~= expected_hash then
          ...
          vim.schedule(function()
            vim.notify("[tetravim] SHA-256 checksum verification failed", vim.log.levels.ERROR)
            cb(false, err_msg)
          end)
          return
        end

        -- Step 5: Install verified binary to stdpath("data")/tetravim/bin/tetravim-engine
        local dest_dir = vim.fn.stdpath("data") .. "/tetravim/bin"
        vim.fn.mkdir(dest_dir, "p")
        local dest_bin = dest_dir .. "/tetravim-engine"
        ...
        vim.schedule(function()
          local succ_msg = string.format("[tetravim] Successfully installed tetravim-engine to %s", dest_bin)
...
local function safe_json_decode(json_str, context, opts)
  ...
  if not ok or type(parsed) ~= "table" then
    if not opts.silent then
      local msg =
        string.format("[tetravim] JSON decode failed%s: %s", context and " (" .. context .. ")" or "", tostring(parsed))
      vim.notify(msg, vim.log.levels.WARN)
    end
    return nil
  end
  ...
  if parsed.success == false then
    if not opts.silent then
      ...
      local msg =
        string.format("[tetravim] %s%s", error_msg, parsed.error_code and (" (" .. parsed.error_code .. ")") or "")
      vim.notify(msg, vim.log.levels.WARN)
    end
    return nil
  end
  ...
  if opts.debug and parsed.success then
    vim.notify(string.format("[tetravim] %s succeeded", context or "command"), vim.log.levels.DEBUG)
  end
...
local function call_engine_command(args, stdin, context, opts)
  ...
  if res.code ~= 0 or not res.stdout or res.stdout == "" then
    if res.code ~= 0 and not opts.silent then
      vim.notify(string.format("[tetravim] engine command failed with exit code %d", res.code), vim.log.levels.WARN)
    end
    return nil
  end
...
function M.classify_workspace(dir, opts)
  ...
  if not ok then
    if not opts.silent then
      vim.notify(string.format("[tetravim] classify-workspace error: %s", err), vim.log.levels.WARN)
    end
    return nil
  end
...
local k8s_ns = vim.api.nvim_create_namespace("tetravim_k8s_validation")
...
local build_ns = vim.api.nvim_create_namespace("tetravim_build")
...
local function register_commands()
  local ok, err = pcall(function()
    vim.api.nvim_create_user_command("TetraVimMetrics", function()
      local metrics = M.get_workspace_cache_metrics()
      local output = {
        "=== TetraVim Engine Metrics ===",
    ...
    end, { desc = "Display TetraVim engine cache metrics and performance stats" })

    vim.api.nvim_create_user_command("TetraVimRefresh", function()
      M.invalidate_workspace_cache()
  ...
  if not ok then
    vim.notify(string.format("[tetravim] Failed to register commands: %s", err), vim.log.levels.WARN)
  end
end
...
--- NOTE: Users can manually clear cache with :TetraVimRefresh for project topology changes
