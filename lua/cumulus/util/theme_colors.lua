...
--- Extract theme colors from engine-generated highlights.
...
function M.init_theme_colors(provider)
  ...
  -- Safe call to generate_theme_highlights with pcall
  local ok, result = pcall(engine.generate_theme_highlights, provider)
  ...
  if type(result.highlights) ~= "table" then
    M.cache = vim.deepcopy(M.DEFAULT_COLORS)
    return
  end
  ...
  -- Extract primary color (provider-specific) from CursorLineNr or Error for fallback
