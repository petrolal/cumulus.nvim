---
description: Create a new task specification in docs/specs/backlog/ (or active/) based on user intent and codebase state
...
By default, new task specifications are created in `docs/specs/backlog/` with status `BACKLOG` (or directly in `docs/specs/active/` with status `ACTIVE` if immediate implementation is requested).
...
4. **Write File to Disk:** Use your file creation tools (`Write`) to save the generated specification into `docs/specs/backlog/<ID>-<feature-name>.md` based on `docs/spec_template.md`.
...
2. Current status (`BACKLOG` or `ACTIVE`).
